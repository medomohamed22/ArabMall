...
// مفتاح API (استبدله بمفتاحك الفعلي من Pi Developer Portal)
const apiKey = "kqnnnmxxu4qyv09dlszoeplomepb7lnq73doeccrlqd2sbthzohdcxd47naj38uq"; // استبدل بمفتاح API الخاص بك
  ...
  campaigns.forEach((campaign) => {
    ...
    const campaignHTML = `
      ...
      currentLang === "ar" ? campaign.name : campaign.nameEn
          ...
          <button class="donate-btn" onclick="promptDonation('${
            campaign.id
          }')">${
...
// دالة لعرض مربع حوار لإدخال مقدار التبرع
async function promptDonation(campaignId) {
  // التحقق من وجود مفتاح API
  if (!apiKey || apiKey === "YOUR_API_KEY_HERE") {
    Swal.fire({
      title: currentLang === "ar" ? "خطأ" : "Error",
      text: currentLang === "ar" ? "يرجى تحديد مفتاح API صالح في الكود" : "Please set a valid API key in the code",
      icon: "error",
    });
    return;
  }
  ...
  const { value: amount } = await Swal.fire({
    title: currentLang === "ar" ? "إدخال مقدار التبرع" : "Enter Donation Amount",
    input: "number",
    inputLabel: currentLang === "ar" ? "المبلغ (Pi)" : "Amount (Pi)",
    inputPlaceholder: currentLang === "ar" ? "أدخل المبلغ" : "Enter amount",
    showCancelButton: true,
    cancelButtonText: currentLang === "ar" ? "إلغاء" : "Cancel",
    confirmButtonText: currentLang === "ar" ? "تأكيد" : "Confirm",
    inputValidator: (value) => {
      if (!value || isNaN(value) || Number(value) <= 0) {
        return currentLang === "ar" ? "يرجى إدخال مبلغ صالح (موجب وغير صفري)" : "Please enter a valid amount (positive and non-zero)";
      }
    },
  });

  if (amount) {
    initiatePayment(campaignId, Number(amount));
  }
}
